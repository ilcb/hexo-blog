...
`POST`、`PUT`请求一般会使用`RequestBody`传递参数，这种情况下后端使用**DTO对象**进行接收。**只要给DTO对象加上`@Validated`注解就能实现自动参数校验**。
...
> **DTO表示数据传输对象（Data Transfer Object），用于服务器和客户端之间交互传输使用**。在spring-web项目中可以表示用于接收请求参数的`Bean`对象。
...
`GET`请求一般会使用`RequestParam/PathVariable`传参，如果参数比较多(比如超过6个)，还是推荐使用`DTO`对象接收。
